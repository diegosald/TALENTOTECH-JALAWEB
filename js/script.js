...
let formC = document.getElementById("form-contacto");
function checkForm(data) {

  let errors = data.entries().filter(e => e[1].length == 0).map(e => e[0].slice(3)).toArray().toString()

if (errors.length > 0) {
  throw new Error("Los Campos " + errors.replace(/,/g, "  ,  ") + " son Obligatorios");
 }
...
async function handleSubmit(event) {
  ...
  //data.entries().forEach(e => console.log(e[1]))
  try {
    checkForm(data)
    let mensaje = await fetch(event.target.action, {
      method: formC.method,
      body: data,
      headers: {
